...
public class MainApp extends Application {
	...
	private static void createTestData() {
		Konference k = Service.createKonference("Baconferencen", "Ham Road 22", LocalDateTime.now().plusDays(3),
				LocalDateTime.now().plusDays(5), "Det handler om bacon, drenge.");

		Udflugt u1 = Service.createUdflugt(k, "U1", "bla", LocalDateTime.now(), LocalDateTime.now(), 120, true);
		Udflugt u2 = Service.createUdflugt(k, "U2", "bllla", LocalDateTime.now(), LocalDateTime.now(), 202, false);
		k.addUdflugt(u1);
		k.addUdflugt(u2);
